...
public class NB_Transition : ScriptableObject
{
    ...
    public void Init(StateMachineSystem stateMachineSystem) 
    {
        ...
        SaveAllStateTransitionInfo(stateMachineSystem);
    }
    ...
    /// 保存所有状态配置信息
    /// </summary>
    private void SaveAllStateTransitionInfo(StateMachineSystem stateMachineSystem) 
    {
        foreach (var item in configStateData)
        {
            ...
            if (!states.ContainsKey(item.fromState)) 
            {
                ...
                item.fromState.Init(stateMachineSystem);
                item.toState.Init(stateMachineSystem);
            }
            else 
            {
                states[item.fromState].Add(item);
