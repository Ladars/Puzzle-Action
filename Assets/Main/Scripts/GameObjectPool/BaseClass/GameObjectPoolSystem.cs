...
public class GameObjectPoolSystem : SingletonBase<GameObjectPoolSystem>
{
    ...
    public GameObjectAssetList gameObjectAssetList;
    ...
    private void InitPool()
    {
        if (gameObjectAssetList._assetsList.Count == 0) return;


        //首先遍历外面配置的资源
        for (int i = 0; i < gameObjectAssetList._assetsList.Count; i++)
        {
            //检查列表第一个元素的内容是否已经在池子里面了，没有的话就创建一个
            if (!pools.ContainsKey(gameObjectAssetList._assetsList[i].assetsName))
            {
                pools.Add(gameObjectAssetList._assetsList[i].assetsName, new Queue<GameObject>());

                if (gameObjectAssetList._assetsList[i].prefab.Length == 0) return;

                //创建完毕后，遍历这个对象的总数，比如总算5，那么就创建5个，然后存进字典
                for (int j = 0; j < gameObjectAssetList._assetsList[i].count; j++)
                {
                    GameObject temp_Gameobject = Instantiate(gameObjectAssetList._assetsList[i].prefab[Random.Range(0, gameObjectAssetList._assetsList[i].prefab.Length)]);
                    temp_Gameobject.transform.SetParent(poolObjectParent);
                    temp_Gameobject.transform.position = Vector3.zero;
                    temp_Gameobject.transform.rotation = Quaternion.identity;
                    pools[gameObjectAssetList._assetsList[i].assetsName].Enqueue(temp_Gameobject);
                    temp_Gameobject.SetActive(false);
                }
            }
        }
    ...
    public GameObject TakeGameObject(string objectName, Vector3 position, Quaternion rotation)
    {
        if (!pools.ContainsKey(objectName)) return null;
