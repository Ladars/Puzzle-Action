...
public class TP_CameraController : MonoBehaviour
{
    ...
    [SerializeField, Header("相机对于玩家")] private Vector2 offsetPlayer;
...
[SerializeField]private float transitionTimeCounter=0;
    ...
    private void Update()
    {
        ...
        AimTransiontionCount();
    }
    private void AimTransiontionCount()
    {
    ...
    private void ControllerCamera()
    {
        ...
        if (_playerInput.playerRAtk)
        {          
            offset += -transform.right * offsetPlayer.x;
            offset += transform.up * offsetPlayer.y;
            offset -= transform.forward * distancePlayerOffset;
            Vector3 fanlePos = LookAttarGet.position + offset ;
            if (transitionTimeCounter > 0)
            {
                transform.position = Vector3.Lerp(transform.position, fanlePos, 10 * Time.deltaTime);
            }
            else
            {
                transform.position = Vector3.Lerp(transform.position, fanlePos, 100 * Time.deltaTime);
            }         
        }
        else
        {
